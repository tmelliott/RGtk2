...
init.stars.array <- function() {
  ...
  {
    stars <<- c(stars, list(list(
      x = x.dist[i], y = y.dist[i],
...
on.expose.event <- function(widget, event) {
  ...
  cr$save()
  cr$translate(
    player1$p$x / FIXED.POINT.SCALE.FACTOR,
  ...
  cr$save()
  cr$translate(
    player2$p$x / FIXED.POINT.SCALE.FACTOR,
  ...
  for (i in 1:MAX.NUMBER.OF.MISSILES)
  {
    if (missiles[[i]]$is.alive) {
      cr$save()
      cr$translate(
        missiles[[i]]$p$x / FIXED.POINT.SCALE.FACTOR,
...
draw.energy.bar <- function(cr, p) {
  ...
  pat$addColorStopRgba(
    0, p$secondary.color$r, p$secondary.color$g,
    p$secondary.color$b, alpha
  )
  pat$addColorStopRgba(
    1, p$primary.color$r, p$primary.color$g,
...
draw.ship.body <- function(cr, p) {
  if (p$is.hit) {
    cr$setSourceRgba(
      p$primary.color$r, p$primary.color$g,
...
draw.star <- function(cr) {
  ...
  cr$moveTo(
    r1 * cos.table[0 * a + 1] / FIXED.POINT.SCALE.FACTOR,
    r1 * sin.table[0 * a + 1] / FIXED.POINT.SCALE.FACTOR
  )
  cr$lineTo(
    r2 * cos.table[1 * a + 1] / FIXED.POINT.SCALE.FACTOR,
    r2 * sin.table[1 * a + 1] / FIXED.POINT.SCALE.FACTOR
  )
  cr$lineTo(
    r1 * cos.table[2 * a + 1] / FIXED.POINT.SCALE.FACTOR,
    r1 * sin.table[2 * a + 1] / FIXED.POINT.SCALE.FACTOR
  )
  cr$lineTo(
    r2 * cos.table[3 * a + 1] / FIXED.POINT.SCALE.FACTOR,
    r2 * sin.table[3 * a + 1] / FIXED.POINT.SCALE.FACTOR
  )
  cr$lineTo(
    r1 * cos.table[4 * a + 1] / FIXED.POINT.SCALE.FACTOR,
    r1 * sin.table[4 * a + 1] / FIXED.POINT.SCALE.FACTOR
  )
  cr$lineTo(
    r2 * cos.table[5 * a + 1] / FIXED.POINT.SCALE.FACTOR,
    r2 * sin.table[5 * a + 1] / FIXED.POINT.SCALE.FACTOR
  )
  cr$lineTo(
    r1 * cos.table[6 * a + 1] / FIXED.POINT.SCALE.FACTOR,
    r1 * sin.table[6 * a + 1] / FIXED.POINT.SCALE.FACTOR
  )
  cr$lineTo(
    r2 * cos.table[7 * a + 1] / FIXED.POINT.SCALE.FACTOR,
    r2 * sin.table[7 * a + 1] / FIXED.POINT.SCALE.FACTOR
  )
  cr$lineTo(
    r1 * cos.table[8 * a + 1] / FIXED.POINT.SCALE.FACTOR,
    r1 * sin.table[8 * a + 1] / FIXED.POINT.SCALE.FACTOR
  )
  cr$lineTo(
    r2 * cos.table[9 * a + 1] / FIXED.POINT.SCALE.FACTOR,
