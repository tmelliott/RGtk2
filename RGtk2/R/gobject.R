...
as.GType <- function(x) {
  mapping <- c(
    "integer" = "gint", "character" = "gchararray", "logical" = "gboolean",
...
format.GType <- function(x, ...) {
  paste("GType:", attr(x, "name"))
}
...
GParamFlags <- c(
  "readable" = 1, "writable" = 2, "construct" = 4,
...
as.GParamSpec <-
  function(x) {
    ...
    if (type %in% c("Boolean", "String", "Unichar")) {
      ...
      "Char", "UChar", "Int", "UInt", "ULong", "Long", "UInt64",
      "Int64", "Float", "Double"
    )) {
...
parentHandler <-
  function(method, obj = NULL, ...) {
    ...
    if (!is.null(parent) && is.function(try(parent[[method]], T))) {
      ...
      eval(substitute(
        gTypeGetClass(class(obj)[2])$sym(obj, ...),
...
"$.GObject" <- "$.RGtkObject" <-
  function(x, member) { # try for a declared method first, else fall back to member
...
"$<-.GObject" <- "[[<-.GObject" <-
  function(obj, member, value) { # first try for prop, then fall back to private env
...
"[[.RGtkObject" <-
  ...
  function(x, field, where = parent.frame()) {
    fun <- try(
      .getAutoElementByName(x, field, error = FALSE, where = where),
      TRUE
    )
...
.getAutoElementByName <-
  function(obj, name, op = "Get", error = TRUE, where = parent.frame()) {
    sym <- paste("S_", class(obj), op,
      toupper(substring(name, 1, 1)), substring(name, 2),
      sep = ""
    )
    ...
    if (is.null(sym)) {
      message <- paste(
        "Could not", op, "field", name, "for classes",
