...
gAsyncInitableNewAsync <-
  function(object.type, io.priority, cancellable, callback, user.data, ...) {
    ...
    w <- .RGtkCall("S_g_async_initable_new_async", object.type, io.priority,
      cancellable, callback, user.data, props,
      PACKAGE = "RGtk2"
    )
...
GFileAttributeMountable <- c(
  canMount = "mountable::can-mount",
...
GPriority <- c(
  default = 0L, high = -100L, highIdle = 100L, defaultIdle = 200L,
  low = 300L
)
