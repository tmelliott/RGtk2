...
.install_system_dependencies <- function() {
  ...
  install_system_dep <- function(dep_name, dep_url, dep_web, installer) {
    ...
    choice <- menu(
      paste(c("Install", "Do not install"), dep_name), T,
  ...
  install_all <- function() {
    ...
    if (is.null(config)) {
      stop(
        "This platform is not yet supported by the automatic installer. ",
