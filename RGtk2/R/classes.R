...
.reserved <- c(
  ".props", ".prop_overrides", ".initialize", ".signals",
...
gClass <- function(name, parent = "GObject", ..., abstract = FALSE) {
  ...
  if (any(!known_types)) {
    warning(
      "The types ", paste(names(types)[!known_types], collapse = ","),
      " are not recognized"
    )
  }
  ...
  if (any(parallel_hierarchies)) {
    stop(
      "The following classes have parallel hierarchies: ",
  ...
  if (any(inconsistents)) {
    stop(
      "Inconsistent hierarchy detected. The classes ",
  ...
  signals <- lapply(class_list$.signals, function(signal) {
    ...
    if (nchar(invalid_chars)) {
      stop(
        "Invalid signal name: ", name, ". Signal names must start with a letter",
  ...
  if (any(sym_dups)) {
    stop(
      "Duplicate symbols in class definition: ",
  ...
  ancestor_syms <- unlist(c(
    mget_null(names(types), .virtuals),
    mget_null(names(types), .fields)
  ))
  ancestor_conflicts <- ancestor_syms %in% syms
  if (any(ancestor_conflicts)) {
    stop(
      "Declared symbols already declared in parent: ",
  ...
  my_virtuals <- list(c(
    public_syms[public_which_funcs],
  ...
  get_class_init_funcs <- function(class_name) {
    paste("S", sapply(class_name, .collapseClassName),
      "class_init",
      sep = "_"
    )
  }
  ...
  if (length(interface_names)) {
    interface_init_syms <- sapply(
      get_class_init_funcs(interface_names),
  ...
  .RGtkCall(
    "S_gobject_class_new", name, parent, interface_names,
