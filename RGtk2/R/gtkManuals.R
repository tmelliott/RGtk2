...
gtkTextBufferInsertWithTags <-
  function(object, iter, text, ...) {
    ...
    w <- .RGtkCall(
      "S_gtk_text_buffer_insert_with_tags", object, iter, text, -1L,
      tags
    )
...
gtkClipboardSetWithOwner <-
  function(object, targets, get.func, owner = NULL) {
    ...
    targets <- checkArrType(targets, function(x) {
      x <- as.GtkTargetEntry(x)
      x
    })
...
gtkWidgetStyleGet <-
  function(object, ...) {
    checkPtrType(object, "GtkWidget")
    props <- c(...)
    w <- sapply(props, function(prop) {
      gtkWidgetStyleGetProperty(object, prop)
    })
    return(invisible(w))
  }
  ...
  gtkTreeModelLoad <-
    function(object, data, rows, cols = 0:(length(data) - 1), paths = NULL, append = F, type = c("list", "tree")) {
      ...
      if (length(cols) != length(data)) {
        stop(
          "The number of specified columns (", length(cols), ")",
      ...
      if (sorted) {
        object$setSortColumnId(-2, sort[["order"]])
      } # -2 means unsorted
